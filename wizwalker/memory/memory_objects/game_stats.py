...
class GameStats(PropertyClass):
    ...
    async def write_energy_max(self, energy_max: int):
        await self.write_value_to_offset(104, energy_max, Primitive.int32)
    ...
    async def write_current_hitpoints(self, current_hitpoints: int):
        await self.write_value_to_offset(108, current_hitpoints, Primitive.int32)
    ...
    async def write_current_gold(self, current_gold: int):
        await self.write_value_to_offset(112, current_gold, Primitive.int32)
    ...
    async def write_current_event_currency1(self, current_event_currency1: int):
        await self.write_value_to_offset(116, current_event_currency1, Primitive.int32)
    ...
    async def write_current_event_currency2(self, current_event_currency2: int):
        await self.write_value_to_offset(120, current_event_currency2, Primitive.int32)
    ...
    async def write_current_pvp_currency(self, current_pvp_currency: int):
        await self.write_value_to_offset(124, current_pvp_currency, Primitive.int32)
    ...
    async def write_current_mana(self, current_mana: int):
        await self.write_value_to_offset(128, current_mana, Primitive.int32)
    ...
    async def write_current_arena_points(self, current_arena_points: int):
        await self.write_value_to_offset(132, current_arena_points, Primitive.int32)

    async def spell_charge_base(self) -> List[int]:
        return await self.read_dynamic_vector(136, Primitive.int32)

    # TODO: add write_dynamic_vector
    # async def write_spell_charge_base(self, spell_charge_base: int):
    #     await self.write_value_to_offset(128, spell_charge_base, Primitive.int32)
    ...
    async def write_potion_max(self, potion_max: float):
        await self.write_value_to_offset(160, potion_max, Primitive.float32)

    async def potion_charge(self) -> float:
        return await self.read_value_from_offset(164, Primitive.float32)

    async def write_potion_charge(self, potion_charge: float):
        await self.write_value_to_offset(164, potion_charge, Primitive.float32)
    ...
    async def write_bonus_hitpoints(self, bonus_hitpoints: int):
        await self.write_value_to_offset(216, bonus_hitpoints, Primitive.int32)

    async def bonus_mana(self) -> int:
        return await self.read_value_from_offset(220, Primitive.int32)

    async def write_bonus_mana(self, bonus_mana: int):
        await self.write_value_to_offset(220, bonus_mana, Primitive.int32)
    ...
    async def write_bonus_energy(self, bonus_energy: int):
        await self.write_value_to_offset(236, bonus_energy, Primitive.int32)

    async def critical_hit_percent_all(self) -> float:
        return await self.read_value_from_offset(240, Primitive.float32)

    async def write_critical_hit_percent_all(self, critical_hit_percent_all: float):
        await self.write_value_to_offset(240, critical_hit_percent_all, Primitive.float32)

    async def block_percent_all(self) -> float:
        return await self.read_value_from_offset(244, Primitive.float32)

    async def write_block_percent_all(self, block_percent_all: float):
        await self.write_value_to_offset(244, block_percent_all, Primitive.float32)
    ...
    async def write_critical_hit_rating_all(self, critical_hit_rating_all: float):
        await self.write_value_to_offset(248, critical_hit_rating_all, Primitive.float32)
    ...
    async def write_block_rating_all(self, block_rating_all: float):
        await self.write_value_to_offset(252, block_rating_all, Primitive.float32)

    async def reference_level(self) -> int:
        return await self.read_value_from_offset(316, Primitive.int32)

    async def write_reference_level(self, reference_level: int):
        await self.write_value_to_offset(316, reference_level, Primitive.int32)

    async def highest_character_level_on_account(self) -> int:
        return await self.read_value_from_offset(328, Primitive.int32)

    async def write_highest_character_level_on_account(self, highest_character_level_on_account: int):
        await self.write_value_to_offset(328, highest_character_level_on_account, Primitive.int32)
    ...
    async def write_pet_act_chance(self, pet_act_chance: int):
        await self.write_value_to_offset(336, pet_act_chance, Primitive.int32)

    async def dmg_bonus_percent(self) -> List[float]:
        return await self.read_dynamic_vector(344, Primitive.float32)

    async def write_dmg_bonus_percent(self, dmg_bonus_percent: float):
        await self.write_value_to_offset(344, dmg_bonus_percent, Primitive.float32)

    async def dmg_bonus_flat(self) -> List[float]:
        return await self.read_dynamic_vector(368, Primitive.float32)

    async def write_dmg_bonus_flat(self, dmg_bonus_flat: float):
        await self.write_value_to_offset(368, dmg_bonus_flat, Primitive.float32)

    async def acc_bonus_percent(self) -> List[float]:
        return await self.read_dynamic_vector(392, Primitive.float32)

    async def write_acc_bonus_percent(self, acc_bonus_percent: float):
        await self.write_value_to_offset(392, acc_bonus_percent, Primitive.float32)

    async def ap_bonus_percent(self) -> List[float]:
        return await self.read_dynamic_vector(416, Primitive.float32)

    async def write_ap_bonus_percent(self, ap_bonus_percent: float):
        await self.write_value_to_offset(416, ap_bonus_percent, Primitive.float32)

    async def dmg_reduce_percent(self) -> List[float]:
        return await self.read_dynamic_vector(440, Primitive.float32)

    async def write_dmg_reduce_percent(self, dmg_reduce_percent: float):
        await self.write_value_to_offset(440, dmg_reduce_percent, Primitive.float32)

    async def dmg_reduce_flat(self) -> List[float]:
        return await self.read_dynamic_vector(464, Primitive.float32)

    async def write_dmg_reduce_flat(self, dmg_reduce_flat: float):
        await self.write_value_to_offset(464, dmg_reduce_flat, Primitive.float32)

    async def acc_reduce_percent(self) -> List[float]:
        return await self.read_dynamic_vector(488, Primitive.float32)

    async def write_acc_reduce_percent(self, acc_reduce_percent: float):
        await self.write_value_to_offset(488, acc_reduce_percent, Primitive.float32)

    async def heal_bonus_percent(self) -> List[float]:
        return await self.read_dynamic_vector(512, Primitive.float32)

    async def write_heal_bonus_percent(self, heal_bonus_percent: float):
        await self.write_value_to_offset(512, heal_bonus_percent, Primitive.float32)

    async def heal_inc_bonus_percent(self) -> List[float]:
        return await self.read_dynamic_vector(536, Primitive.float32)

    async def write_heal_inc_bonus_percent(self, heal_inc_bonus_percent: float):
        await self.write_value_to_offset(536, heal_inc_bonus_percent, Primitive.float32)

    async def spell_charge_bonus(self) -> List[int]:
        return await self.read_dynamic_vector(584, Primitive.int32)

    async def write_spell_charge_bonus(self, spell_charge_bonus: int):
        await self.write_value_to_offset(584, spell_charge_bonus, Primitive.int32)

    async def dmg_bonus_percent_all(self) -> float:
        return await self.read_value_from_offset(704, Primitive.float32)

    async def write_dmg_bonus_percent_all(self, dmg_bonus_percent_all: float):
        await self.write_value_to_offset(704, dmg_bonus_percent_all, Primitive.float32)

    async def dmg_bonus_flat_all(self) -> float:
        return await self.read_value_from_offset(708, Primitive.float32)

    async def write_dmg_bonus_flat_all(self, dmg_bonus_flat_all: float):
        await self.write_value_to_offset(708, dmg_bonus_flat_all, Primitive.float32)
    ...
    async def write_acc_bonus_percent_all(self, acc_bonus_percent_all: float):
        await self.write_value_to_offset(712, acc_bonus_percent_all, Primitive.float32)
    ...
    async def write_ap_bonus_percent_all(self, ap_bonus_percent_all: float):
        await self.write_value_to_offset(716, ap_bonus_percent_all, Primitive.float32)
    ...
    async def write_dmg_reduce_percent_all(self, dmg_reduce_percent_all: float):
        await self.write_value_to_offset(720, dmg_reduce_percent_all, Primitive.float32)
    ...
    async def write_dmg_reduce_flat_all(self, dmg_reduce_flat_all: float):
        await self.write_value_to_offset(724, dmg_reduce_flat_all, Primitive.float32)
    ...
    async def write_acc_reduce_percent_all(self, acc_reduce_percent_all: float):
        await self.write_value_to_offset(728, acc_reduce_percent_all, Primitive.float32)
    ...
    async def write_heal_bonus_percent_all(self, heal_bonus_percent_all: float):
        await self.write_value_to_offset(732, heal_bonus_percent_all, Primitive.float32)
    ...
    async def write_heal_inc_bonus_percent_all(self, heal_inc_bonus_percent_all: float):
        await self.write_value_to_offset(736, heal_inc_bonus_percent_all, Primitive.float32)
    ...
    async def write_spell_charge_bonus_all(self, spell_charge_bonus_all: int):
        await self.write_value_to_offset(744, spell_charge_bonus_all, Primitive.int32)
    ...
    async def write_power_pip_base(self, power_pip_base: float):
        await self.write_value_to_offset(748, power_pip_base, Primitive.float32)

    async def power_pip_bonus_percent_all(self) -> float:
        return await self.read_value_from_offset(784, Primitive.float32)

    async def write_power_pip_bonus_percent_all(self, power_pip_bonus_percent_all: float):
        await self.write_value_to_offset(784, power_pip_bonus_percent_all, Primitive.float32)
    ...
    async def write_xp_percent_increase(self, xp_percent_increase: float):
        await self.write_value_to_offset(792, xp_percent_increase, Primitive.float32)

    async def critical_hit_percent_by_school(self) -> List[float]:
        return await self.read_dynamic_vector(608, Primitive.float32)

    async def write_critical_hit_percent_by_school(self, critical_hit_percent_by_school: float):
        await self.write_value_to_offset(608, critical_hit_percent_by_school, Primitive.float32)

    async def block_percent_by_school(self) -> List[float]:
        return await self.read_dynamic_vector(632, Primitive.float32)

    async def write_block_percent_by_school(self, block_percent_by_school: float):
        await self.write_value_to_offset(632, block_percent_by_school, Primitive.float32)

    async def critical_hit_rating_by_school(self) -> List[float]:
        return await self.read_dynamic_vector(656, Primitive.float32)

    async def write_critical_hit_rating_by_school(self, critical_hit_rating_by_school: float):
        await self.write_value_to_offset(656, critical_hit_rating_by_school, Primitive.float32)

    async def block_rating_by_school(self) -> List[float]:
        return await self.read_dynamic_vector(680, Primitive.float32)

    async def write_block_rating_by_school(self, block_rating_by_school: float):
        await self.write_value_to_offset(680, block_rating_by_school, Primitive.float32)

    async def balance_mastery(self) -> int:
        return await self.read_value_from_offset(820, Primitive.int32)

    async def write_balance_mastery(self, balance_mastery: int):
        await self.write_value_to_offset(820, balance_mastery, Primitive.int32)

    async def death_mastery(self) -> int:
        return await self.read_value_from_offset(824, Primitive.int32)

    async def write_death_mastery(self, death_mastery: int):
        await self.write_value_to_offset(824, death_mastery, Primitive.int32)

    async def fire_mastery(self) -> int:
        return await self.read_value_from_offset(828, Primitive.int32)

    async def write_fire_mastery(self, fire_mastery: int):
        await self.write_value_to_offset(828, fire_mastery, Primitive.int32)
    ...
    async def write_ice_mastery(self, ice_mastery: int):
        await self.write_value_to_offset(832, ice_mastery, Primitive.int32)
    ...
    async def write_life_mastery(self, life_mastery: int):
        await self.write_value_to_offset(836, life_mastery, Primitive.int32)
    ...
    async def write_myth_mastery(self, myth_mastery: int):
        await self.write_value_to_offset(840, myth_mastery, Primitive.int32)
    ...
    async def write_storm_mastery(self, storm_mastery: int):
        await self.write_value_to_offset(844, storm_mastery, Primitive.int32)
    ...
    async def write_maximum_number_of_islands(self, maximum_number_of_islands: int):
        await self.write_value_to_offset(848, maximum_number_of_islands, Primitive.int32)

    async def gardening_level(self) -> int:
        return await self.read_value_from_offset(852, Primitive.uint8)

    async def write_gardening_level(self, gardening_level: int):
        await self.write_value_to_offset(852, gardening_level, Primitive.uint8)
    ...
    async def write_gardening_xp(self, gardening_xp: int):
        await self.write_value_to_offset(856, gardening_xp, Primitive.int32)

    async def invisible_to_friends(self) -> bool:
        return await self.read_value_from_offset(860, Primitive.bool)

    async def write_invisible_to_friends(self, invisible_to_friends: bool):
        await self.write_value_to_offset(860, invisible_to_friends, Primitive.bool)

    async def show_item_lock(self) -> bool:
        return await self.read_value_from_offset(861, Primitive.bool)

    async def write_show_item_lock(self, show_item_lock: bool):
        await self.write_value_to_offset(861, show_item_lock, Primitive.bool)

    async def quest_finder_enabled(self) -> bool:
        return await self.read_value_from_offset(862, Primitive.bool)

    async def write_quest_finder_enabled(self, quest_finder_enabled: bool):
        await self.write_value_to_offset(862, quest_finder_enabled, Primitive.bool)

    async def buddy_list_limit(self) -> int:
        return await self.read_value_from_offset(864, Primitive.int32)

    async def write_buddy_list_limit(self, buddy_list_limit: int):
        await self.write_value_to_offset(864, buddy_list_limit, Primitive.int32)
    ...
    async def write_dont_allow_friend_finder_codes(self, dont_allow_friend_finder_codes: bool):
        await self.write_value_to_offset(872, dont_allow_friend_finder_codes, Primitive.bool)
    ...
    async def write_stun_resistance_percent(self, stun_resistance_percent: float):
        await self.write_value_to_offset(868, stun_resistance_percent, Primitive.float32)
    ...
    async def write_shadow_magic_unlocked(self, shadow_magic_unlocked: bool):
        await self.write_value_to_offset(880, shadow_magic_unlocked, Primitive.bool)
    ...
    async def write_shadow_pip_max(self, shadow_pip_max: int):
        await self.write_value_to_offset(876, shadow_pip_max, Primitive.int32)

    async def fishing_level(self) -> int:
        return await self.read_value_from_offset(881, Primitive.uint8)

    async def write_fishing_level(self, fishing_level: int):
        await self.write_value_to_offset(881, fishing_level, Primitive.uint8)

    async def fishing_xp(self) -> int:
        return await self.read_value_from_offset(884, Primitive.int32)

    async def write_fishing_xp(self, fishing_xp: int):
        await self.write_value_to_offset(884, fishing_xp, Primitive.int32)

    async def fishing_luck_bonus_percent(self) -> List[float]:
        return await self.read_dynamic_vector(560, Primitive.float32)

    async def write_fishing_luck_bonus_percent(self, fishing_luck_bonus_percent: float):
        await self.write_value_to_offset(560, fishing_luck_bonus_percent, Primitive.float32)
    ...
    async def write_fishing_luck_bonus_percent_all(self, fishing_luck_bonus_percent_all: float):
        await self.write_value_to_offset(740, fishing_luck_bonus_percent_all, Primitive.float32)
    ...
    async def write_subscriber_benefit_flags(self, subscriber_benefit_flags: int):
        await self.write_value_to_offset(888, subscriber_benefit_flags, Primitive.uint32)
    ...
    async def write_elixir_benefit_flags(self, elixir_benefit_flags: int):
        await self.write_value_to_offset(892, elixir_benefit_flags, Primitive.uint32)

    async def shadow_pip_bonus_percent(self) -> float:
        return await self.read_value_from_offset(788, Primitive.float32)

    async def write_shadow_pip_bonus_percent(self, shadow_pip_bonus_percent: float):
        await self.write_value_to_offset(788, shadow_pip_bonus_percent, Primitive.float32)

    async def wisp_bonus_percent(self) -> float:
        return await self.read_value_from_offset(812, Primitive.float32)

    async def write_wisp_bonus_percent(self, wisp_bonus_percent: float):
        await self.write_value_to_offset(812, wisp_bonus_percent, Primitive.float32)

    async def pip_conversion_rating_all(self) -> float:
        return await self.read_value_from_offset(280, Primitive.float32)

    async def write_pip_conversion_rating_all(self, pip_conversion_rating_all: float):
        await self.write_value_to_offset(280, pip_conversion_rating_all, Primitive.float32)

    async def pip_conversion_rating_per_school(self) -> List[float]:
        return await self.read_dynamic_vector(256, Primitive.float32)

    async def write_pip_conversion_rating_per_school(self, pip_conversion_rating_per_school: float):
        await self.write_value_to_offset(256, pip_conversion_rating_per_school, Primitive.float32)

    async def pip_conversion_percent_all(self) -> float:
        return await self.read_value_from_offset(312, Primitive.float32)

    async def write_pip_conversion_percent_all(self, pip_conversion_percent_all: float):
        await self.write_value_to_offset(312, pip_conversion_percent_all, Primitive.float32)

    async def pip_conversion_percent_per_school(self) -> List[float]:
        return await self.read_dynamic_vector(288, Primitive.float32)

    async def write_pip_conversion_percent_per_school(self, pip_conversion_percent_per_school: float):
        await self.write_value_to_offset(288, pip_conversion_percent_per_school, Primitive.float32)

    async def monster_magic_level(self) -> int:
        return await self.read_value_from_offset(896, Primitive.uint8)

    async def write_monster_magic_level(self, monster_magic_level: int):
        await self.write_value_to_offset(896, monster_magic_level, Primitive.uint8)

    async def monster_magic_xp(self) -> int:
        return await self.read_value_from_offset(900, Primitive.int32)

    async def write_monster_magic_xp(self, monster_magic_xp: int):
        await self.write_value_to_offset(900, monster_magic_xp, Primitive.int32)
    ...
    async def write_player_chat_channel_is_public(self, player_chat_channel_is_public: bool):
        await self.write_value_to_offset(904, player_chat_channel_is_public, Primitive.bool)
    ...
    async def write_extra_inventory_space(self, extra_inventory_space: int):
        await self.write_value_to_offset(908, extra_inventory_space, Primitive.int32)

    async def remember_last_realm(self) -> bool:
        return await self.read_value_from_offset(912, Primitive.bool)

    async def write_remember_last_realm(self, remember_last_realm: bool):
        await self.write_value_to_offset(912, remember_last_realm, Primitive.bool)

    async def new_spellbook_layout_warning(self) -> bool:
        return await self.read_value_from_offset(913, Primitive.bool)

    async def write_new_spellbook_layout_warning(self, new_spellbook_layout_warning: bool):
        await self.write_value_to_offset(913, new_spellbook_layout_warning, Primitive.bool)
    ...
    async def write_pip_conversion_base_all_schools(self, pip_conversion_base_all_schools: int):
        await self.write_value_to_offset(752, pip_conversion_base_all_schools, Primitive.int32)

    async def pip_conversion_base_per_school(self) -> List[int]:
        return await self.read_dynamic_vector(760, Primitive.int32)

    async def write_pip_conversion_base_per_school(self, pip_conversion_base_per_school: int):
        await self.write_value_to_offset(760, pip_conversion_base_per_school, Primitive.int32)

    async def purchased_custom_emotes1(self) -> int:
        return await self.read_value_from_offset(916, Primitive.uint32)

    async def write_purchased_custom_emotes1(self, purchased_custom_emotes1: int):
        await self.write_value_to_offset(916, purchased_custom_emotes1, Primitive.uint32)

    async def purchased_custom_teleport_effects1(self) -> int:
        return await self.read_value_from_offset(920, Primitive.uint32)

    async def write_purchased_custom_teleport_effects1(self, purchased_custom_teleport_effects1: int):
        await self.write_value_to_offset(920, purchased_custom_teleport_effects1, Primitive.uint32)

    async def equipped_teleport_effect(self) -> int:
        return await self.read_value_from_offset(924, Primitive.uint32)

    async def write_equipped_teleport_effect(self, equipped_teleport_effect: int):
        await self.write_value_to_offset(924, equipped_teleport_effect, Primitive.uint32)
    ...
    async def write_highest_world1_id(self, highest_world1_id: int):
        await self.write_value_to_offset(944, highest_world1_id, Primitive.uint32)
    ...
    async def write_highest_world2_id(self, highest_world2_i_d: int):
        await self.write_value_to_offset(948, highest_world2_i_d, Primitive.uint32)
    ...
    async def write_active_class_projects_list(self, active_class_projects_list: int):
        await self.write_value_to_offset(952, active_class_projects_list, Primitive.uint32)
    ...
    async def write_disabled_item_slot_ids(self, disabled_item_slot_ids: int):
        await self.write_value_to_offset(968, disabled_item_slot_ids, Primitive.uint32)
    ...
    async def write_adventure_power_cooldown_time(self, adventure_power_cooldown_time: int):
        await self.write_value_to_offset(984, adventure_power_cooldown_time, Primitive.uint32)
    ...
    async def write_purchased_custom_emotes2(self, purchased_custom_emotes2: int):
        await self.write_value_to_offset(928, purchased_custom_emotes2, Primitive.uint32)
    ...
    async def write_purchased_custom_teleport_effects2(self, purchased_custom_teleport_effects2: int):
        await self.write_value_to_offset(932, purchased_custom_teleport_effects2, Primitive.uint32)
    ...
    async def write_purchased_custom_emotes3(self, purchased_custom_emotes3: int):
        await self.write_value_to_offset(936, purchased_custom_emotes3, Primitive.uint32)
    ...
    async def write_purchased_custom_teleport_effects3(self, purchased_custom_teleport_effects3: int):
        await self.write_value_to_offset(940, purchased_custom_teleport_effects3, Primitive.uint32)

    async def shadow_pip_rating(self) -> float:
        return await self.read_value_from_offset(988, Primitive.float32)

    async def write_shadow_pip_rating(self, shadow_pip_rating: float):
        await self.write_value_to_offset(988, shadow_pip_rating, Primitive.float32)

    async def bonus_shadow_pip_rating(self) -> float:
        return await self.read_value_from_offset(992, Primitive.float32)

    async def write_bonus_shadow_pip_rating(self, bonus_shadow_pip_rating: float):
        await self.write_value_to_offset(992, bonus_shadow_pip_rating, Primitive.float32)

    async def shadow_pip_rate_accumulated(self) -> float:
        return await self.read_value_from_offset(996, Primitive.float32)

    async def write_shadow_pip_rate_accumulated(self, shadow_pip_rate_accumulated: float):
        await self.write_value_to_offset(996, shadow_pip_rate_accumulated, Primitive.float32)

    async def shadow_pip_rate_threshold(self) -> float:
        return await self.read_value_from_offset(1000, Primitive.float32)

    async def write_shadow_pip_rate_threshold(self, shadow_pip_rate_threshold: float):
        await self.write_value_to_offset(1000, shadow_pip_rate_threshold, Primitive.float32)

    async def shadow_pip_rate_percentage(self) -> int:
        return await self.read_value_from_offset(1004, Primitive.int32)

    async def write_shadow_pip_rate_percentage(self, shadow_pip_rate_percentage: int):
        await self.write_value_to_offset(1004, shadow_pip_rate_percentage, Primitive.int32)

    async def friendly_player(self) -> bool:
        return await self.read_value_from_offset(1008, Primitive.bool)

    async def write_friendly_player(self, friendly_player: bool):
        await self.write_value_to_offset(1008, friendly_player, Primitive.bool)

    async def emoji_skin_tone(self) -> int:
        return await self.read_value_from_offset(1012, Primitive.int32)

    async def write_emoji_skin_tone(self, emoji_skin_tone: int):
        await self.write_value_to_offset(1012, emoji_skin_tone, Primitive.int32)

    async def show_pvp_option(self) -> int:
        return await self.read_value_from_offset(1016, Primitive.uint32)

    async def write_show_pvp_option(self, show_pvp_option: int):
        await self.write_value_to_offset(1016, show_pvp_option, Primitive.uint32)
    ...
    async def write_favorite_slot(self, favorite_slot: int):
        await self.write_value_to_offset(1020, favorite_slot, Primitive.int32)

    async def cantrip_level(self) -> int:
        return await self.read_value_from_offset(1024, Primitive.uint8)

    async def write_cantrip_level(self, cantrip_level: int):
        await self.write_value_to_offset(1024, cantrip_level, Primitive.uint8)
    ...
    async def write_cantrip_xp(self, cantrip_xp: int):
        await self.write_value_to_offset(1028, cantrip_xp, Primitive.int32)

    async def base_archmastery_rating(self) -> float:
        return await self.read_value_from_offset(1032, Primitive.float32)

    async def write_base_archmastery_rating(self, base_archmastery_rating: float):
        return await self.write_value_to_offset(1032, base_archmastery_rating, Primitive.float32)

    async def bonus_archmastery_rating(self) -> float:
        return await self.read_value_from_offset(1036, Primitive.float32)

    async def write_bonus_archmastery_rating(self, bonus_archmastery_rating: float):
        return await self.write_value_to_offset(1036, bonus_archmastery_rating, Primitive.float32)
    ...
    async def write_highest_character_world_on_account(self, highest_character_world_on_account: int):
        return await self.write_value_to_offset(332, highest_character_world_on_account, Primitive.int32)

    async def school_id(self) -> int:
        return await self.read_value_from_offset(320, Primitive.uint32)

    async def write_school_id(self, school_id: int):
        return await self.write_value_to_offset(320, school_id, Primitive.uint32)
    ...
    async def write_level_scaled(self, level_scaled: int):
        return await self.write_value_to_offset(324, level_scaled, Primitive.int32)
