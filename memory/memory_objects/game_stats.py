...
class GameStats(PropertyClass):
    ...
    async def write_highest_character_level_on_account(
        ...
        await self.write_value_to_offset(328, highest_character_level_on_account, "int")

    async def pet_act_chance(self) -> int:
        return await self.read_value_from_offset(336, "int")

    async def write_pet_act_chance(self, pet_act_chance: int):
        await self.write_value_to_offset(336, pet_act_chance, "int")

    async def dmg_bonus_percent(self) -> List[float]:
        return await self.read_dynamic_vector(344, "float")

    async def write_dmg_bonus_percent(self, dmg_bonus_percent: float):
        await self.write_value_to_offset(344, dmg_bonus_percent, "float")

    async def dmg_bonus_flat(self) -> List[float]:
        return await self.read_dynamic_vector(368, "float")

    async def write_dmg_bonus_flat(self, dmg_bonus_flat: float):
        await self.write_value_to_offset(368, dmg_bonus_flat, "float")

    async def acc_bonus_percent(self) -> List[float]:
        return await self.read_dynamic_vector(392, "float")

    async def write_acc_bonus_percent(self, acc_bonus_percent: float):
        await self.write_value_to_offset(392, acc_bonus_percent, "float")

    async def ap_bonus_percent(self) -> List[float]:
        return await self.read_dynamic_vector(416, "float")

    async def write_ap_bonus_percent(self, ap_bonus_percent: float):
        await self.write_value_to_offset(416, ap_bonus_percent, "float")

    async def dmg_reduce_percent(self) -> List[float]:
        return await self.read_dynamic_vector(440, "float")

    async def write_dmg_reduce_percent(self, dmg_reduce_percent: float):
        await self.write_value_to_offset(440, dmg_reduce_percent, "float")

    async def dmg_reduce_flat(self) -> List[float]:
        return await self.read_dynamic_vector(464, "float")

    async def write_dmg_reduce_flat(self, dmg_reduce_flat: float):
        await self.write_value_to_offset(464, dmg_reduce_flat, "float")

    async def acc_reduce_percent(self) -> List[float]:
        return await self.read_dynamic_vector(488, "float")

    async def write_acc_reduce_percent(self, acc_reduce_percent: float):
        await self.write_value_to_offset(488, acc_reduce_percent, "float")

    async def heal_bonus_percent(self) -> List[float]:
        return await self.read_dynamic_vector(512, "float")

    async def write_heal_bonus_percent(self, heal_bonus_percent: float):
        await self.write_value_to_offset(512, heal_bonus_percent, "float")

    async def heal_inc_bonus_percent(self) -> List[float]:
        return await self.read_dynamic_vector(536, "float")

    async def write_heal_inc_bonus_percent(self, heal_inc_bonus_percent: float):
        await self.write_value_to_offset(536, heal_inc_bonus_percent, "float")

    async def spell_charge_bonus(self) -> List[int]:
        return await self.read_dynamic_vector(584, "int")

    async def write_spell_charge_bonus(self, spell_charge_bonus: int):
        await self.write_value_to_offset(584, spell_charge_bonus, "int")
    ...
    async def write_dmg_bonus_percent_all(self, dmg_bonus_percent_all: float):
        await self.write_value_to_offset(704, dmg_bonus_percent_all, "float")
    ...
    async def write_ap_bonus_percent_all(self, ap_bonus_percent_all: float):
        await self.write_value_to_offset(716, ap_bonus_percent_all, "float")
    ...
    async def write_dmg_reduce_percent_all(self, dmg_reduce_percent_all: float):
        await self.write_value_to_offset(720, dmg_reduce_percent_all, "float")
    ...
    async def write_dmg_reduce_flat_all(self, dmg_reduce_flat_all: float):
        await self.write_value_to_offset(724, dmg_reduce_flat_all, "float")
    ...
    async def write_acc_reduce_percent_all(self, acc_reduce_percent_all: float):
        await self.write_value_to_offset(728, acc_reduce_percent_all, "float")
    ...
    async def write_heal_bonus_percent_all(self, heal_bonus_percent_all: float):
        await self.write_value_to_offset(732, heal_bonus_percent_all, "float")

    async def heal_inc_bonus_percent_all(self) -> float:
        return await self.read_value_from_offset(736, "float")

    async def write_heal_inc_bonus_percent_all(self, heal_inc_bonus_percent_all: float):
        await self.write_value_to_offset(736, heal_inc_bonus_percent_all, "float")
    ...
    async def write_spell_charge_bonus_all(self, spell_charge_bonus_all: int):
        await self.write_value_to_offset(744, spell_charge_bonus_all, "int")

    async def power_pip_base(self) -> float:
        return await self.read_value_from_offset(748, "float")

    async def write_power_pip_base(self, power_pip_base: float):
        await self.write_value_to_offset(748, power_pip_base, "float")
    ...
    async def write_power_pip_bonus_percent_all(
        ...
        await self.write_value_to_offset(784, power_pip_bonus_percent_all, "float")

    async def xp_percent_increase(self) -> float:
        return await self.read_value_from_offset(792, "float")

    async def write_xp_percent_increase(self, xp_percent_increase: float):
        await self.write_value_to_offset(792, xp_percent_increase, "float")

    async def critical_hit_percent_by_school(self) -> List[float]:
        return await self.read_dynamic_vector(608, "float")

    async def write_critical_hit_percent_by_school(
        self, critical_hit_percent_by_school: float
    ):
        await self.write_value_to_offset(608, critical_hit_percent_by_school, "float")

    async def block_percent_by_school(self) -> List[float]:
        return await self.read_dynamic_vector(632, "float")

    async def write_block_percent_by_school(self, block_percent_by_school: float):
        await self.write_value_to_offset(632, block_percent_by_school, "float")

    async def critical_hit_rating_by_school(self) -> List[float]:
        return await self.read_dynamic_vector(656, "float")

    async def write_critical_hit_rating_by_school(
        self, critical_hit_rating_by_school: float
    ):
        await self.write_value_to_offset(656, critical_hit_rating_by_school, "float")

    async def block_rating_by_school(self) -> List[float]:
        return await self.read_dynamic_vector(680, "float")

    async def write_block_rating_by_school(self, block_rating_by_school: float):
        await self.write_value_to_offset(680, block_rating_by_school, "float")
    ...
    async def write_balance_mastery(self, balance_mastery: int):
        await self.write_value_to_offset(820, balance_mastery, "int")
    ...
    async def write_death_mastery(self, death_mastery: int):
        await self.write_value_to_offset(824, death_mastery, "int")
    ...
    async def write_fire_mastery(self, fire_mastery: int):
        await self.write_value_to_offset(828, fire_mastery, "int")
    ...
    async def write_ice_mastery(self, ice_mastery: int):
        await self.write_value_to_offset(832, ice_mastery, "int")
    ...
    async def write_life_mastery(self, life_mastery: int):
        await self.write_value_to_offset(836, life_mastery, "int")
    ...
    async def write_myth_mastery(self, myth_mastery: int):
        await self.write_value_to_offset(840, myth_mastery, "int")
    ...
    async def write_storm_mastery(self, storm_mastery: int):
        await self.write_value_to_offset(844, storm_mastery, "int")
    ...
    async def write_maximum_number_of_islands(self, maximum_number_of_islands: int):
        await self.write_value_to_offset(848, maximum_number_of_islands, "int")

    async def gardening_level(self) -> int:
        return await self.read_value_from_offset(852, "unsigned char")

    async def write_gardening_level(self, gardening_level: int):
        await self.write_value_to_offset(852, gardening_level, "unsigned char")
    ...
    async def write_gardening_xp(self, gardening_xp: int):
        await self.write_value_to_offset(856, gardening_xp, "int")

    async def invisible_to_friends(self) -> bool:
        return await self.read_value_from_offset(860, "bool")

    async def write_invisible_to_friends(self, invisible_to_friends: bool):
        await self.write_value_to_offset(860, invisible_to_friends, "bool")

    async def show_item_lock(self) -> bool:
        return await self.read_value_from_offset(861, "bool")

    async def write_show_item_lock(self, show_item_lock: bool):
        await self.write_value_to_offset(861, show_item_lock, "bool")

    async def quest_finder_enabled(self) -> bool:
        return await self.read_value_from_offset(862, "bool")

    async def write_quest_finder_enabled(self, quest_finder_enabled: bool):
        await self.write_value_to_offset(862, quest_finder_enabled, "bool")

    async def buddy_list_limit(self) -> int:
        return await self.read_value_from_offset(864, "int")
    ...
    async def write_stun_resistance_percent(self, stun_resistance_percent: float):
        await self.write_value_to_offset(868, stun_resistance_percent, "float")

    async def shadow_magic_unlocked(self) -> bool:
        return await self.read_value_from_offset(880, "bool")

    async def write_shadow_magic_unlocked(self, shadow_magic_unlocked: bool):
        await self.write_value_to_offset(880, shadow_magic_unlocked, "bool")
    ...
    async def write_shadow_pip_max(self, shadow_pip_max: int):
        await self.write_value_to_offset(876, shadow_pip_max, "int")

    async def fishing_level(self) -> int:
        return await self.read_value_from_offset(881, "unsigned char")

    async def write_fishing_level(self, fishing_level: int):
        await self.write_value_to_offset(881, fishing_level, "unsigned char")
    ...
    async def write_fishing_xp(self, fishing_xp: int):
        await self.write_value_to_offset(884, fishing_xp, "int")

    async def fishing_luck_bonus_percent(self) -> List[float]:
        return await self.read_dynamic_vector(560, "float")

    async def write_fishing_luck_bonus_percent(self, fishing_luck_bonus_percent: float):
        await self.write_value_to_offset(560, fishing_luck_bonus_percent, "float")
    ...
    async def write_fishing_luck_bonus_percent_all(
        ...
        await self.write_value_to_offset(740, fishing_luck_bonus_percent_all, "float")
    ...
    async def write_subscriber_benefit_flags(self, subscriber_benefit_flags: int):
        await self.write_value_to_offset(888, subscriber_benefit_flags, "unsigned int")
    ...
    async def write_elixir_benefit_flags(self, elixir_benefit_flags: int):
        await self.write_value_to_offset(892, elixir_benefit_flags, "unsigned int")

    async def shadow_pip_bonus_percent(self) -> float:
        return await self.read_value_from_offset(788, "float")

    async def write_shadow_pip_bonus_percent(self, shadow_pip_bonus_percent: float):
        await self.write_value_to_offset(788, shadow_pip_bonus_percent, "float")

    async def wisp_bonus_percent(self) -> float:
        return await self.read_value_from_offset(812, "float")

    async def write_wisp_bonus_percent(self, wisp_bonus_percent: float):
        await self.write_value_to_offset(812, wisp_bonus_percent, "float")
    ...
    async def monster_magic_level(self) -> int:
        return await self.read_value_from_offset(896, "unsigned char")

    async def write_monster_magic_level(self, monster_magic_level: int):
        await self.write_value_to_offset(896, monster_magic_level, "unsigned char")
    ...
    async def write_monster_magic_xp(self, monster_magic_xp: int):
        await self.write_value_to_offset(900, monster_magic_xp, "int")
    ...
    async def write_player_chat_channel_is_public(
        ...
        await self.write_value_to_offset(904, player_chat_channel_is_public, "bool")

    async def extra_inventory_space(self) -> int:
        return await self.read_value_from_offset(908, "int")

    async def write_extra_inventory_space(self, extra_inventory_space: int):
        await self.write_value_to_offset(908, extra_inventory_space, "int")

    async def remember_last_realm(self) -> bool:
        return await self.read_value_from_offset(912, "bool")

    async def write_remember_last_realm(self, remember_last_realm: bool):
        await self.write_value_to_offset(912, remember_last_realm, "bool")

    async def new_spellbook_layout_warning(self) -> bool:
        return await self.read_value_from_offset(913, "bool")

    async def write_new_spellbook_layout_warning(
        self, new_spellbook_layout_warning: bool
    ):
        await self.write_value_to_offset(913, new_spellbook_layout_warning, "bool")

    async def pip_conversion_base_all_schools(self) -> int:
        return await self.read_value_from_offset(752, "int")

    async def write_pip_conversion_base_all_schools(
        self, pip_conversion_base_all_schools: int
    ):
        await self.write_value_to_offset(752, pip_conversion_base_all_schools, "int")

    async def pip_conversion_base_per_school(self) -> List[int]:
        return await self.read_dynamic_vector(760, "int")

    async def write_pip_conversion_base_per_school(
        self, pip_conversion_base_per_school: int
    ):
        await self.write_value_to_offset(760, pip_conversion_base_per_school, "int")
    ...
    async def write_purchased_custom_emotes1(self, purchased_custom_emotes1: int):
        await self.write_value_to_offset(916, purchased_custom_emotes1, "unsigned int")
    ...
    async def write_purchased_custom_teleport_effects1(
        ...
        await self.write_value_to_offset(
            920, purchased_custom_teleport_effects1, "unsigned int"
        )
    ...
    async def write_shadow_pip_rating(self, shadow_pip_rating: float):
        await self.write_value_to_offset(988, shadow_pip_rating, "float")
    ...
    async def write_bonus_shadow_pip_rating(self, bonus_shadow_pip_rating: float):
        await self.write_value_to_offset(992, bonus_shadow_pip_rating, "float")

    async def shadow_pip_rate_accumulated(self) -> float:
        return await self.read_value_from_offset(996, "float")

    async def write_shadow_pip_rate_accumulated(
        self, shadow_pip_rate_accumulated: float
    ):
        await self.write_value_to_offset(996, shadow_pip_rate_accumulated, "float")

    async def shadow_pip_rate_threshold(self) -> float:
        return await self.read_value_from_offset(1000, "float")

    async def write_shadow_pip_rate_threshold(self, shadow_pip_rate_threshold: float):
        await self.write_value_to_offset(1000, shadow_pip_rate_threshold, "float")
    ...
    async def write_shadow_pip_rate_percentage(self, shadow_pip_rate_percentage: int):
        await self.write_value_to_offset(1004, shadow_pip_rate_percentage, "int")

    async def friendly_player(self) -> bool:
        return await self.read_value_from_offset(1008, "bool")

    async def write_friendly_player(self, friendly_player: bool):
        await self.write_value_to_offset(1008, friendly_player, "bool")
    ...
    async def write_emoji_skin_tone(self, emoji_skin_tone: int):
        await self.write_value_to_offset(1012, emoji_skin_tone, "int")

    async def show_pvp_option(self) -> int:
        return await self.read_value_from_offset(1016, "unsigned int")

    async def write_show_pvp_option(self, show_pvp_option: int):
        await self.write_value_to_offset(1016, show_pvp_option, "unsigned int")
    ...
    async def write_favorite_slot(self, favorite_slot: int):
        await self.write_value_to_offset(1020, favorite_slot, "int")

    async def cantrip_level(self) -> int:
        return await self.read_value_from_offset(1024, "unsigned char")

    async def write_cantrip_level(self, cantrip_level: int):
        await self.write_value_to_offset(1024, cantrip_level, "unsigned char")

    async def cantrip_xp(self) -> int:
        return await self.read_value_from_offset(1028, "int")

    async def write_cantrip_xp(self, cantrip_xp: int):
        await self.write_value_to_offset(1028, cantrip_xp, "int")

    async def base_archmastery_rating(self) -> float:
        return await self.read_value_from_offset(1032, "float")

    async def write_base_archmastery_rating(self, base_archmastery_rating: float):
        return await self.write_value_to_offset(1032, base_archmastery_rating, "float")

    async def bonus_archmastery_rating(self) -> float:
        return await self.read_value_from_offset(1036, "float")

    async def write_bonus_archmastery_rating(self, bonus_archmastery_rating: float):
        return await self.write_value_to_offset(1036, bonus_archmastery_rating, "float")

    async def highest_character_world_on_account(self) -> int:
        return await self.read_value_from_offset(332, "int")
